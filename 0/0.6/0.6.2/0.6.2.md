...
// Variables de configuración inyectadas por entorno/CI
const PORT = process.env.PORT || 3000;
const API_BASE_URL = process.env.API_BASE_URL || "https://example.local"; // provista vía Secrets en CI
...
// Servidor en memoria para prueba de humo
function createApp() {
  const app = express();
...
jobs:
  build-test:
    ...
    env:
      # Inyección de configuración desde Secrets (definidos en el repo)
...
> **Secrets utilizados:** `API_BASE_URL` (URL base para consumo de servicios externos durante pruebas y verificación de salud). Se agregó en **Settings → Secrets and variables → Actions** del repositorio. El secreto no aparece en _logs_ gracias al _masking_ automático de GitHub Actions.

#### 5) Evidencias y validación
- **Ejecución local**: `/health` responde `200` con `{ ok: true, apiBase: "…" }`.  
- **CI en GitHub**: se observan las etapas `Install deps`, `Run tests` y `Build image` con _status_ **verde** en el historial de _Actions_.  
